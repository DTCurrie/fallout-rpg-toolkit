...
import {
  ...
  CombatRollOptions,
  CombatRollResult,
} from "@fallout-rpg-toolkit/dice-roller/dist/combat-roll";

import {
  DamageEffect,
  DamageEffectType,
} from "@fallout-rpg-toolkit/rules/dist/damage/damage-effect";

import { DamageType } from "@fallout-rpg-toolkit/rules/dist/damage/damage";

import {
  getHitLocationText,
  HitLocation,
  HitLocationType,
} from "@fallout-rpg-toolkit/rules/dist/hit-locations";

import { capitalize } from "@fallout-rpg-toolkit/utils/dist/text/capitalize";
...
| --------------------------------------------------------------------- |
    ...
    if (combatNotationRegex.test(formula)) {
      try {
        ...
        return showError(error as Error);
      }
    }
