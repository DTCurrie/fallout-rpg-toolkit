...
import {
  ...
  SkillRollOptions,
  SkillRollResult,
} from "@fallout-rpg-toolkit/dice-roller/dist/skill-roll";

import { getNextSymbolOrSpace } from "@fallout-rpg-toolkit/utils/dist/text/parse";
    ...
    new Menu(message.channel as TextChannel, message.author.id, [
      {
        ...
        content: new MessageEmbed({
          ...
          fields: [
            {
              name: "Successes",
              value: `${successes}/${options.difficulty} ${
                complicated ? " ⚠️" : ""
              }`,
              inline: true,
            },
...
| ----------------------------------------- |
    ...
    if (skillNotationRegex.test(formula)) {
      ...
      const difficulty = (difficultyString && parseInt(difficultyString)) || 0;
